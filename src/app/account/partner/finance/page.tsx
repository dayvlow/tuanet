...
import { PartnerFinance } from "@/components/account/PartnerFinance";
import {
    BackendAccount,
...
export default async function PartnerFinancePage() {
    ...
    return (
        <AccountShell
            title="Финансы"
            description="Комиссия, заявки на вывод и выплаты"
        ...
        >
            ...
            {state === "error" && (
                <div className="rounded-[32px] border-2 border-red-500/30 bg-red-500/10 p-8 text-white">
                    ...
                    <div className="mt-4 text-3xl font-black uppercase tracking-tight">Не удалось загрузить финансы</div>
                    <div className="mt-2 text-sm text-white/70">
            ...
            {state === "success" && overview && <PartnerFinance overview={overview} />}
        </AccountShell>
    );
}
